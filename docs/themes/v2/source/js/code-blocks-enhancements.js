...
(function () {
  ...
  const addCopyCodeButton = (codeBlock, language) => {
    ...
    const htmlTemplate = `
      <button class="code-block-copy-button" arial-label="复制${language}代码">
        ${language}
  ...
  const addPlaygroundButtons = (codeBlock) => {
    ...
    <div class="playground-buttons">
      <button class="code-block-open-preview">打开预览</button>
      <a href="/playground/?config=${encodeConfig(code)}" target="_blank" rel="noreferrer noopener">在游乐场中启动</a>
    </div>
    `;
