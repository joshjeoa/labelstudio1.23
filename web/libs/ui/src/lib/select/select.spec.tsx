...
describe("Select Component", () => {
  ...
  describe("Basic Rendering", () => {
    it("renders with placeholder", () => {
      render(<Select options={["Apple", "Banana"]} placeholder="选择一种水果" />);
      expect(screen.getByText("Select a fruit")).toBeInTheDocument();
    });

    it("renders with flat options", async () => {
      render(<Select options={["Apple", "Banana", "Cherry"]} placeholder="选择" />);
    ...
    it("renders with grouped options", async () => {
      const groupedOptions = [
        { label: "水果", children: ["Apple", "Banana"] },
        { label: "蔬菜", children: ["Carrot", "Broccoli"] },
      ];

      render(<Select options={groupedOptions as any} placeholder="选择" />);
  ...
  describe("Virtual List Height Calculation", () => {
    ...
    it("calculates correct height for flat options with virtual list", async () => {
      ...
      render(<Select options={flatOptions as any} isVirtualList={true} searchable={true} placeholder="选择" />);
    ...
    it("calculates correct height for many flat options (capped at max visible)", async () => {
      ...
      render(<Select options={manyOptions as any} isVirtualList={true} searchable={true} placeholder="选择" />);
    ...
    it("calculates correct height for grouped options using flatOptions count", async () => {
      ...
      render(<Select options={groupedOptions as any} isVirtualList={true} searchable={true} placeholder="选择" />);
    ...
    it("calculates correct height when searching flat options", async () => {
      ...
      render(<Select options={options as any} isVirtualList={true} searchable={true} placeholder="选择" />);
    ...
    it("calculates correct height when searching grouped options", async () => {
      const groupedOptions = [
        { label: "水果", children: ["Apple", "Apricot", "Banana"] },
        { label: "蔬菜", children: ["Asparagus", "Artichoke", "Broccoli"] },
      ];
    ...
    it("renders footer content when provided", async () => {
      render(
        <Select
          ...
          placeholder="选择"
          footer={<div data-testid="custom-footer">自定义页脚</div>}
        />,
      );
  ...
  describe("Selection Behavior", () => {
    it("selects an option when clicked", async () => {
      ...
      render(
        <Select options={["Apple", "Banana", "Cherry"] as any} placeholder="选择一种水果" onChange={onChange} />,
      );
  ...
  describe("Edge Cases", () => {
    it("handles empty options", async () => {
      render(<Select options={[] as any} placeholder="无选项" searchable={true} />);
    ...
    it("handles disabled state", () => {
      render(<Select options={["Apple", "Banana"] as any} placeholder="选择" disabled={true} />);
  ...
  describe("Label and description", () => {
    it("renders with label and description when provided", () => {
      render(
        <Select options={["A", "B"] as any} placeholder="挑选" label="选择一个" description="可选的辅助文本" />,
      );
      expect(screen.getByText("选择一个")).toBeInTheDocument();
      expect(screen.getByText("可选的辅助文本")).toBeInTheDocument();
    });
  });
  ...
  describe("Search and callbacks", () => {
    it("calls onSearch when search input changes", async () => {
      ...
      render(
        <Select options={["Apple", "Banana"] as any} placeholder="选择" searchable={true} onSearch={onSearch} />,
      );
      fireEvent.click(screen.getByRole("button"));
      const searchInput = screen.getByTestId("select-search-field");
  ...
  describe("Open/close callbacks", () => {
    it("calls onOpen when dropdown opens and onClose when option selected (single)", async () => {
      ...
      render(<Select options={["Apple", "Banana"] as any} placeholder="选择" onOpen={onOpen} onClose={onClose} />);
      fireEvent.click(screen.getByRole("button"));
  ...
  describe("renderSelected and selectFirstIfEmpty", () => {
    it("uses renderSelected when provided", async () => {
      ...
      render(<Select options={["Apple", "Banana"] as any} placeholder="挑选一个" renderSelected={renderSelected} />);
      expect(renderSelected).toHaveBeenCalled();
      fireEvent.click(screen.getByRole("button"));
      await waitFor(() => expect(screen.getByText("Apple")).toBeInTheDocument());
      fireEvent.click(screen.getByText("Apple"));
      await waitFor(() => expect(renderSelected).toHaveBeenCalledWith(expect.any(Array), "挑选一个"));
    });
  ...
  describe("Loading and trigger attributes", () => {
    it("shows Loading when isLoading", async () => {
      render(<Select options={["A", "B"] as any} placeholder="选择" isLoading={true} />);
      fireEvent.click(screen.getByRole("button"));
      await waitFor(() => expect(screen.getByText("Loading...")).toBeInTheDocument());
    });

    it("applies data-testid from props when provided", () => {
      render(<Select options={["A"] as any} placeholder="选择" dataTestid="my-select" />);
  ...
  describe("Option keyboard navigation", () => {
    it("selects option on Enter key", async () => {
      const onChange = jest.fn();
      render(<Select options={["Apple", "Banana"] as any} placeholder="选择" onChange={onChange} />);
      fireEvent.click(screen.getByRole("button"));
      await waitFor(() => expect(screen.getByText("Apple")).toBeInTheDocument());
    ...
    it("selects option on Space key", async () => {
      const onChange = jest.fn();
      render(<Select options={["Banana"] as any} placeholder="选择" onChange={onChange} />);
      fireEvent.click(screen.getByRole("button"));
  ...
  describe("Group option with multiple", () => {
    it("toggles all children when clicking group header in multiple mode", async () => {
      ...
      render(<Select options={options as any} placeholder="选择" multiple={true} onChange={onChange} />);
      fireEvent.click(screen.getByRole("button"));
  ...
  describe("Controlled value (externalValue)", () => {
    it("syncs to external value when value prop changes", async () => {
      const { rerender } = render(
        <Select options={["Apple", "Banana", "Cherry"] as any} placeholder="Sel选择ect" value="Apple" />,
      );
      expect(screen.getByText("Apple")).toBeInTheDocument();
      rerender(<Select options={["Apple", "Banana", "Cherry"] as any} placeholder="选择" value="Banana" />);
      await waitFor(() => expect(screen.getByText("Banana")).toBeInTheDocument());
    });
    ...
    it("syncs to external value object with value key", () => {
      render(
        <Select
          options={["Apple", "Banana"] as any}
          placeholder="选择"
          value={{ value: "苹果", label: "Apple" } as any}
        />,
      );
      expect(screen.getByText("Apple")).toBeInTheDocument();
    });
  ...
  describe("Option keyboard ArrowDown and ArrowUp", () => {
    it("moves focus to next option on ArrowDown", async () => {
      render(<Select options={["Apple", "Banana", "Cherry"] as any} placeholder="选择" />);
      fireEvent.click(screen.getByRole("button"));
      await waitFor(() => expect(screen.getByText("Apple")).toBeInTheDocument());
  ...
  describe("Single select with grouped options", () => {
    it("renders group label as non-interactive header when not multiple", async () => {
      ...
      render(<Select options={options as any} placeholder="选择" />);
      fireEvent.click(screen.getByRole("button"));
      await waitFor(() => expect(screen.getByText("Fruits")).toBeInTheDocument());
  ...
  describe("Trigger size and props", () => {
    it("applies size small to trigger", () => {
      render(<Select options={["A"] as any} placeholder="选择" size="small" />);
      const trigger = screen.getByRole("button");
      expect(trigger).toHaveClass("sizeSmall");
    });

    it("applies size large to trigger", () => {
      render(<Select options={["A"] as any} placeholder="选择" size="large" />);
      const trigger = screen.getByRole("button");
      expect(trigger).toHaveClass("sizeLarge");
    });

    it("spreads triggerProps onto trigger", () => {
      render(<Select options={["A"] as any} placeholder="选择" triggerProps={{ "data-foo": "bar" }} />);
  ...
  describe("onChange when disabled", () => {
    it("does not call onChange when selecting and component is disabled", async () => {
      const onChange = jest.fn();
      render(<Select options={["Apple", "Banana"] as any} placeholder="选择" disabled={true} onChange={onChange} />);
      const trigger = screen.getByRole("button");
      expect(trigger).toBeDisabled();
    });
  });
  ...
  describe("Native select and form", () => {
    it("renders hidden native select with name and value for form submission", async () => {
      render(<Select options={["Apple", "Banana"] as any} placeholder="选择" name="fruit" />);
      fireEvent.click(screen.getByRole("button"));
      await waitFor(() => expect(screen.getByText("Apple")).toBeInTheDocument());
      fireEvent.click(screen.getByText("Apple"));
