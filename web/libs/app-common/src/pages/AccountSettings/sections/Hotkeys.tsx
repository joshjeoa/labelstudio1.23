  ...
  const handleImportHotkeys = async (importedData: ImportData | Hotkey[]) => {
    try {
      ...
      if (toast) {
        ...
        toast.show({ message: `导入热键出错：${errorMessage}`, type: ToastType.error });
      }
    } finally {
      setIsLoading(false);
    }
    ...
    <div id="hotkeys-manager">
      ...
      <Dialog open={duplicateConfirmDialog.open} onOpenChange={handleCancelDuplicate}>
        <DialogContent className="bg-neutral-surface">
          <DialogHeader>
            <DialogTitle>警告：检测到重复的热键</DialogTitle>
            <DialogDescription>
              快捷键组合“<strong>{duplicateConfirmDialog.newKey}</strong>”已被以下项占用：
            </DialogDescription>
          </DialogHeader>
