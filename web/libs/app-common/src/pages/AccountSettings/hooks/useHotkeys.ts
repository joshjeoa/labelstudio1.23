...
export const useHotkeys = () => {
  ...
  const loadHotkeysFromAPI = useCallback(async () => {
    try {
      ...
      if (toast) {
        toast.show({
          message: "无法从服务器加载自定义热键，正在使用缓存设置",
  ...
  const saveHotkeysToAPI = useCallback(
    async (currentHotkeys: Hotkey[], currentSettings: HotkeySettings): Promise<SaveResult> => {
      ...
      try {
        ...
        console.error(`错误${operation}热键：`, error);

        // Provide more specific error messages
        let errorMessage = `无法${isReset ? "重置" : "保存"}热键`;
        if (error && typeof error === "object" && "response" in error) {
          const err = error as any;
          // Server responded with error status
          if (err.response?.status === 400) {
            errorMessage = err.response.data?.error || `无效的 ${isReset ? "重置请求" : "热键配置"}`;
  ...
  const handleResetToDefaults = useCallback(() => {
    confirm({
      title: "将热键重置为默认值？",
      body: "您确定要将所有热键和设置重置为默认值吗？此操作无法撤销。",
      okText: "恢复默认设置",
      buttonLook: "negative",
      style: { width: 500 },
      onOk: async () => {
  ...
  const handleImportHotkeys = useCallback(
    async (importedData: ImportData | Hotkey[]) => {
      try {
        ...
        if (!result.ok) {
          throw new Error(result.error || "保存导入的热键失败");
        }
