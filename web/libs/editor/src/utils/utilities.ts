...
export const humanDateDiff = (date: string | number): string => {
  ...
  let str = fnsDate
    .replace(/less than a minute ago/, "刚刚")
    .replace(/(\d+) minute(s)? ago/, "$1分钟前")
    .replace(/about (\d+) hour(s)? ago/, "大约$1小时前")
    .replace(/(\d+) day(s)? ago/, "$1天前")
    .replace(/(\d+) week(s)? ago/, "$1周前")
    .replace(/(\d+) month(s)? ago/, "$1个月前")
    .replace(/(\d+) year(s)? ago/, "$1年前");
