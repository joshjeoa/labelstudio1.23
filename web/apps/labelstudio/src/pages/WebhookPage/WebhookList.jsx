    ...
    >
      <div>
        ...
        <div className="text-neutral-content-subtler text-sm mt-1">
          创建日期 {format(new Date(webhook.created_at), "yyyy-MM-dd HH:mm")}
        </div>
      </div>
      {canChangeWebhooks && (
      ...
      <header className="mb-base">
        ...
        {webhooks.length > 0 && (
          <Typography size="small" className="text-neutral-content-subtler">
            设置订阅特定事件的集成，使用 Webhooks。当事件被触发时，{"app name"}{" "}
            会向配置的 Webhook URL 发一个 HTTP POST 请求。
          </Typography>
        )}
      </header>
        ...
        {webhooks.length === 0 ? (
          <SimpleCard title="" className="bg-primary-background border-primary-border-subtler p-base">
            <EmptyState
              ...
              description="设置订阅特定事件的集成，使用 Webhooks。当事件被触发时，Label Studio 会向配置的 Webhook URL 发送一个 HTTP POST 请求。"
              actions={
                canChangeWebhooks ? (
                ...
                !window.APP_SETTINGS.whitelabel_is_active && (
                  <Typography variant="label" size="small" className="text-primary-link">
                    <a
                      ...
                      aria-label="了解更多关于 Webhook 的信息（在新窗口中打开）"
                    >
                      了解更多
