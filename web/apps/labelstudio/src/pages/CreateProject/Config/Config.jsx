...
const EmptyConfigPlaceholder = () => (
  <div className={configClass.elem("empty-config").toClassName()}>
    <p>您的标注配置为空。标注您的数据是必需的。</p>
    <p>
      从我们的预定义模板之一开始，或在代码面板上创建您自己的配置。标注配置是基于XML的，您可以{" "}
...
const ConfigureColumn = ({ template, obj, columns }) => {
  ...
  const displayName = tagTranslateMap[obj.tagName.toLowerCase()] ?? obj.tagName.toLowerCase();
...
const ConfigureColumns = ({ columns, template }) => {
  ...
  return (
    <div className={configClass.elem("object").toClassName()}>
      ...
      {columns?.length === 0 && (
        <p className={configClass.elem("object-error").toClassName()}>
          要选择要标记的字段，您需要上传数据。或者，您也可以使用代码模式提供数据。
        </p>
      )}
...
export const ConfigPage = ({
  ...
  const setTemplate = React.useCallback(
    (newConfig) => {
      // 强制给所有配置注入固定View属性
      let fixedConfig = newConfig || EMPTY_CONFIG;
      const forceAttrs = 'strokeWidth="1" imageZoom="true" showZoomControls="true" showRotateControls="true" labelPosition="top" enableLabelFilter="true"';

      // 匹配<View标签，追加/覆盖属性
      if (fixedConfig.includes("<View")) {
        // 替换原有<View 开头标签，强制写入固定属性
        fixedConfig = fixedConfig.replace(/<View(\s[^>]*?)?>/, `<View ${forceAttrs}>`);
      } else {
        // 不存在View标签，外层包裹View
        fixedConfig = `<View ${forceAttrs}>${fixedConfig}</View>`;
      }

      setConfig(fixedConfig);
      try {
        const tpl = new Template({ config: fixedConfig });
