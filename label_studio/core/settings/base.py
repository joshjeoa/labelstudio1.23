...
# 兼容本地localhost:8010 + 局域网192.168.110.161:8010
FRONTEND_HOSTNAME = get_env('FRONTEND_HOSTNAME', 'http://localhost:8010,http://192.168.110.161:8010' if FRONTEND_HMR else HOSTNAME)
...
CSRF_TRUSTED_ORIGINS = [
    "http://192.168.110.161:8010",
    "http://192.168.110.161:8080",
    "http://localhost:8010",
    "http://localhost:8080",
    "http://127.0.0.1:8010",
    "http://127.0.0.1:8080"
...
# ===================== SESSION Cookie 跨域兼容配置 =====================
# SameSite=None 允许localhost与192网段跨IP传递Cookie
SESSION_COOKIE_SAMESITE = "Lax"
CSRF_COOKIE_SAMESITE = "Lax"
# 内网HTTP非HTTPS，关闭Secure
SESSION_COOKIE_SECURE = False
CSRF_COOKIE_SECURE = False
# Cookie不绑定固定域名，本地/局域网IP均可读取
...
# Sessions and CSRF 已统一在上方CORS区域配置，此处不再重复
...
# Public base URL for relative satellite data paths such as /unzip/... .
SATELLITE_DATA_URL = get_env('SATELLITE_DATA_URL', 'http://101.33.242.54:9000/satellite-data/')
...
if get_env('STORAGE_TYPE') == 's3':
    ...
    AWS_S3_VERIFY = get_env('STORAGE_AWS_VERIFY', None)
...
# ===================== CSP 完整兼容本地+远程，消除控制台所有资源拦截报错 =====================
if ENABLE_CSP := get_bool_env('ENABLE_CSP', True):
    # 统一放行本地localhost、127.0.0.1、局域网后端IP
    ALLOW_HOSTS = (
        "'self'",
        "'report-sample'",
        "http://localhost:8010",
    ...
    CSP_DEFAULT_SRC = ALLOW_HOSTS
    CSP_STYLE_SRC = (*ALLOW_HOSTS, "'unsafe-inline'")
    CSP_FONT_SRC = ALLOW_HOSTS
    CSP_SCRIPT_SRC = (
        *ALLOW_HOSTS,
        "'unsafe-inline'",
        'blob:',
