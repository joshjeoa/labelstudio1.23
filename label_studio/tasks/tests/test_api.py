...
class TestTaskAPIResolveUri(APITestCase):
    ...
    def test_get_task_resolves_relative_unzip_path_to_satellite_storage(self):
        task = TaskFactory(
            project=self.project,
            data={'image': '/unzip/20260721/job-id/images/example.jpg'},
        )
        self.client.force_authenticate(user=self.user)
        ...
        assert response.json()['data']['image'] == (
            'http://101.33.242.54:9000/satellite-data/'
            'unzip/20260721/job-id/images/example.jpg'
        )
