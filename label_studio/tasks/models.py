...
class Task(TaskMixin, FsmHistoryStateModel):
    ...
    def resolve_uri(self, task_data, project):
        ...
        if project.task_data_login and project.task_data_password:
            ...
            # Relative satellite paths would otherwise be resolved by the browser against
            # /projects/<id>/ and return 404. Convert them to the public object-storage URL.
            for field, value in task_data.items():
                if isinstance(value, str) and value.lstrip('/').startswith('unzip/'):
                    task_data[field] = urljoin(settings.SATELLITE_DATA_URL.rstrip('/') + '/', value.lstrip('/'))
